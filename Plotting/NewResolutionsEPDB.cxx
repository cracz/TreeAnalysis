...
void NewResolutionsEPDB(TString jobID)
{
  ...
  for (int i = 1; i <= centBins; i++)
    {
      ...
      R11_epdB  = TMath::Sqrt( (EpdAEpdB * TpcBEpdB) / TpcBEpdA);
      dR11_epdB = R11_epdB * TMath::Sqrt((dEpdAEpdB/(2*EpdAEpdB))*(dEpdAEpdB/(2*EpdAEpdB)) +
					 (dTpcBEpdA/(2*TpcBEpdA))*(dTpcBEpdA/(2*TpcBEpdA)) +
					 (dTpcBEpdB/(2*TpcBEpdB))*(dTpcBEpdB/(2*TpcBEpdB)));

      if(!TMath::IsNaN(R11_epdB))
	{
	  v_R11.push_back(R11_epdB);
	  v_dR11.push_back(dR11_epdB);
	  v_R11plus.push_back(R11_epdB + dR11_epdB);
	  v_R11minus.push_back(R11_epdB - dR11_epdB);
	}
      else
	{
	  v_R11.push_back(0.0);
  ...
  */
  ...
  TH1D *h_resolutions_R11_3sub = new TH1D("h_resolutions_R11_3sub","EPD B 3-sub Resolutions;Centrality;R_{11}",centBins,0,centBins);
  ...
  canvas->SaveAs(jobID + "_resolutionBonly_R11.pdf");
  canvas->Clear();

  gr_R13->SetTitle("");
  gr_R13->Draw("ACP");

  canvas->SaveAs(jobID + "_resolutionBonly_R13.pdf");
  canvas->Clear();
